...
export default async function handler(req, res) {
  ...
  const status = await new Promise(resolve => {
    const socket = net.createConnection({ host: '199.83.103.156', port: 25612 });
    ...
    socket.on('connect', () => {
      const host = Buffer.from('199.83.103.156');
      const packet = Buffer.concat([varInt(0), varInt(771), varInt(host.length), host, Buffer.from([0x64, 0x0c]), varInt(1)]);
